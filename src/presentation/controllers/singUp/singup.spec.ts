...
import { MissParamError, InvalidParamError, ServerError } from '../../errors'
...
describe('SingUp Controller', () => {
  ...
  test('Should return 400 if password it not equal to confirmation', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Should return 400 if passwordConfirmation no provided', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
    ...
    const httpResponse = await sut.handle(httpRequest)
    expect(httpResponse.statusCode).toBe(400)
    expect(httpResponse.body).toEqual(new MissParamError('passwordConfirmation'))
  })
  ...
  test('Should return 500 if AddAccount throw error', async () => {
    const { sut, addAccountStub } = makeSut()
